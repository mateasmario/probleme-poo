...
2. Metoda `deblocheaza(Cheie cheie)`, care verifică dacă identificatorul cheii din parametru este același cu identificatorul cheii din proprietatea seifului. Dacă se apelează metoda cu o cheie validă, seiful va fi deblocat și va putea fi deschis din nou cu cifrul corect. Metoda returnează `true`, dacă deblocarea s-a făcut cu succes. Altfel, aceasta returnează `false`.
3. Metoda `schimbaCheie(Cheie cheieVeche, Cheie cheieNoua)`, care verifică dacă identificatorul cheii vechi este același cu cel din proprietatea seifului. În cazul în care cheia veche este validă, se va înlocui identificatorul din proprietate cu cel al cheii noi.
4. Metoda `schimbaCifru(Cheie cheie, int cifru)`, care verifică validitatea cheii, iar în cazul în care se dă cheia corectă, setează noul cifru pe cel dat ca parametru.
